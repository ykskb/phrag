...
(defmethod ig/init-key ::list-root [_ {:keys [db table cols]}]
  (fn [{[_ query] :ataraxy/result}]
    (println (query->filters query cols))
    (let [res (db/list db table (query->filters query cols))]
      [::response/ok res])))
...
(defmethod ig/init-key ::fetch-root [_ {:keys [db table cols]}]
  (fn [{[_ id {:as query}] :ataraxy/result}]
    (println id)
    (let [res (db/fetch db table id (query->filters query cols))]
      [::response/ok res])))
...
(defmethod ig/init-key ::fetch-one-n [_ {:keys [db table p-col cols]}]
  (fn [{[_ p-id id {:as query}] :ataraxy/result}]
    (let [filters (query->filters (assoc query p-col p-id) cols)
          res (db/fetch db table id filters)]
      [::response/ok res])))

(defmethod ig/init-key ::delete-one-n [_ {:keys [db table p-col cols]}]
  (fn [{[_ p-id id] :ataraxy/result}]
    ; TODO: handle p-id check
    (let [res (db/delete! db table id p-col p-id)]
      [::response/ok res])))

(defmethod ig/init-key ::put-one-n [_ {:keys [db table p-col cols]}]
  (fn [{[_ p-id id {:as params}] :ataraxy/result}]
  ; TODO: params to be cover all the attributes as PUT updates all attributes
    (println "ID: " id "DATA:" params)
    (println (db/update! db table id params p-col p-id))
    [::response/ok {:result "UPDATED"}]))

(defmethod ig/init-key ::patch-one-n [_ {:keys [db table p-col cols]}]
  (fn [{[_ p-id id {:as params}] :ataraxy/result}]
    (println (db/update! db table id params p-col p-id))
    [::response/ok {:result "UPDATED"}]))
...
(defmethod ig/init-key ::delete-n-n [_ {:keys [db table col-a col-b cols]}]
  (fn [{[_ id-a id-b] :ataraxy/result}]
    (let [filters (query->filters {col-a id-a col-b id-b} cols)]
      [::response/ok (db/delete-where! db table filters)])))
