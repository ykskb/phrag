  ...
  (testing "root resource routes"
    (let [db (create-database)
      ...
      (let [params-a {"first_name" "john"
        ...
        (testing "list returns by order"
          (is (= (ring-res/response [created-b created-a])
                 (list-hdlr {:query-string "order-by=id:desc"}))))
        (testing "list returns 2nd item by order and limit"
          (is (= (ring-res/response [created-b])
                 (list-hdlr {:query-string "order-by=id:desc&limit=1"}))))
        (testing "list returns 2nd item with limit and offset"
          (is (= (ring-res/response [created-b])
                 (list-hdlr {:query-string "limit=1&offset=1"}))))
        (testing "list returns 1st item with offset and orderby"
          (is (= (ring-res/response [created-a])
                 (list-hdlr {:query-string "order-by=id:desc&limit=1&offset=1"}))))
  ...
  (testing "one-n resource routes"
    (let [db (create-database)
      ...
      (let [params-a {"title" "user group meetup Jan"
        ...
        (testing "list returns by order"
          (is (= (ring-res/response [created-b updated-a])
                 (list-hdlr {:route-params {:p-id "2"}
                             :query-string "order-by=id:desc"}))))
        (testing "list returns 2nd item by order and limit"
          (is (= (ring-res/response [created-b])
                 (list-hdlr {:route-params {:p-id "2"}
                             :query-string "order-by=id:desc&limit=1"}))))
        (testing "list returns 2nd item with limit and offset"
          (is (= (ring-res/response [created-b])
                 (list-hdlr {:route-params {:p-id "2"}
                             :query-string "limit=1&offset=1"}))))
        ;; Test deletion
        (testing "delete returns 200"
          (is (= (ring-res/response nil)
                 (delete-hdlr {:route-params {:p-id "2" :id "1"}}))))
        (testing "delete returns 200"
          (is (= (ring-res/response nil)
                 (delete-hdlr {:route-params {:p-id "2" :id "2"}}))))
